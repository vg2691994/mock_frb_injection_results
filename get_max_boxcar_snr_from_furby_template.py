...
def main():
  furby_db = args.furby_db
  furby_cat_file = furby_db + "/furbies.cat"
  furby_ids = np.loadtxt(furby_cat_file, usecols=0, skiprows=4, dtype=str)#, max_rows=100)
  outfile = args.outfile

  rms_noise_per_channel = 1.0
  boxcars = np.arange(args.min_bw, args.max_bw, 1)

  HDR_str = "Furby_file\tHDR_SNR\tHDR_width\tBest_SNR\tBest_width\tBest_loc\tBoxcar_{}\n".format("\tBoxcar_".join(map(str, boxcars)))
  of = open(outfile, 'w')
  of.write(HDR_str)

  for furby_id in tqdm(furby_ids):
    furby_file = furby_db + "/furby_" + furby_id
    ff = F(furby_file)
    nch = ff.header.NCHAN
